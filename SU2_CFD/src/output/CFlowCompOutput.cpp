...
void CFlowCompOutput::SetVolumeOutputFields(CConfig *config){
  ...
  if (config->GetKind_Solver() == NAVIER_STOKES){
    switch (config->GetKind_SGS_Model()) {
      case AMD: case VREMAN: case WALE: case SMAGORINSKY: case SIGMA:
...
void CFlowCompOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  if (config->GetKind_RoeLowDiss() != NO_ROELOWDISS){
    if (config->GetKind_RoeLowDiss() == DUCROS && (config->GetHybrid_Central_Upwind() || config->GetKind_Centered_Flow() == JST_KE)) {
